...
export default function ManageLanding({ routines, currentRoutine, currentRoutineSecondary }: Props) {
    ...
    return (
        <main className={`main centeredFlex`}>
            ...
            <Card className={`${styles.selectCard} mainButton`}
                clickable
                onClick={() => setOption(1)}>
                <input id="selectRoutine"
                    name="landingRoutingRadio"
                    checked={option === 1}
                    onChange={() => setOption(1)}
                    type='radio'
                    className={styles.selectCardItem}></input>
                <label htmlFor='selectRoutine'
                    className={styles.selectCardItem}>
                    Select current routine </label>
            </Card>
            <Card
                className={`${styles.selectCard} mainButton`}
                clickable
                onClick={() => setOption(2)}>
                <input id="updateRoutine"
                    name="landingRoutingRadio"
                    type='radio'
